...
public class VinNumberValidator implements ConstraintValidator<VinNumber, String> {
    ...
    public boolean isValid(String vin, ConstraintValidatorContext context) {
        ...
        if (vin.length() != 17) {
            setMessage(context, "VIN number must be exactly 17 characters long");
            return false;
        }
        ...
        for (int i = 0; i < vin.length(); i++) {
            ...
            if (!TRANSLITERATION.containsKey(c)) {
                setMessage(context, "VIN number contains invalid characters");
                return false;
            }
        ...
        if (vin.charAt(8) != expectedCheckDigit) {
            setMessage(context, "Invalid VIN number check digit");
            return false;
        }
